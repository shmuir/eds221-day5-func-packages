...
# The line below creating a time vector from year 0 to 8 by 1s.
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below is making an empty matrix based on the lengths of fish_year0 and time_yr
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below fill in first row with fish_year0 (the initial conditions) since we already know the values for year 0
proj_year[1, ] <- fish_year0
proj_year

# The for loop below fills proj_year with the dot product of fish_leslie and proj_year (previous year values). Starting at row 2 since we already filled row 1 with the year 0 initial condition data
